...
Null_Hypothesis = 'Reject' if chi2 > critical_value else 'Accept'
...
rpred=ridge_grid.predict(X_test)

ridge_rmse=np.sqrt(mean_squared_error(y_test,rpred))
print(ridge_rmse)

lasso_model=Lasso()
lasso_grid=GridSearchCV(estimator=lasso_model, param_grid=dict(alpha=lasso_lambdas))
lasso_grid.fit(X_train, y_train)
lpred=lasso_grid.predict(X_test)

lasso_rmse=np.sqrt(mean_squared_error(y_test,lpred))
print(lasso_rmse)
